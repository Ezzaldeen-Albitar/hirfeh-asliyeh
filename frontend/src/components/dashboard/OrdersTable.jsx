...
export default function OrdersTable({ orders = [], onStatusChange }) {
  if (!orders.length) return (
    <div className="text-center py-5" style={{color:'var(--warm-gray)'}}>
      <i className="bi bi-inbox fs-1 d-block mb-2"/>لا توجد طلبات
    </div>
  );
          ...
          {orders.map(o => {
            ...
            return (
              <tr key={o._id} style={{borderBottom:'1px solid var(--gold-pale)'}}>
                ...
                {onStatusChange && (
                  <td className="px-3">
                    <select className="form-select form-select-sm" style={{borderRadius:8,fontSize:'0.8rem',width:'auto'}}
                      value={o.status} onChange={e => onStatusChange(o._id, e.target.value)}>
