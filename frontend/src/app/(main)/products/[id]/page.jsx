...
export default function ProductDetailPage() {
      ...
      <div className="container" style={{padding:'48px 12px'}}>
        <div className="row g-5">
          <div className="col-lg-5">
            <div className="placeholder-glow"><div className="placeholder w-100 rounded-3" style={{height:400,background:'var(--parchment)'}}/></div>
          </div>
  ...
  if (isError || !product) return (
    <div className="text-center py-5" style={{color:'var(--warm-gray)'}}>
      ...
      <Link href="/products" className="btn btn-primary mt-3" style={{borderRadius:10}}>العودة للمنتجات</Link>
    </div>
  );
          ...
          <ol className="breadcrumb mb-0" style={{fontSize:'0.82rem'}}>
            ...
            {product.artisan && (
              <li className="breadcrumb-item">
                <Link href={`/artisans/${product.artisan._id}`} style={{color:'var(--warm-gray)',textDecoration:'none'}}>{product.artisan.name}</Link>
              </li>
            )}
          ...
          <div className="col-lg-7">
            ...
            {product.artisan && (
              <Link href={`/artisans/${product.artisan._id}`}
                ...
                <div style={{width:52,height:52,borderRadius:"inherit",overflow:"hidden",position:"relative",flexShrink:0}}><Image src={product.artisan.avatar} alt={product.artisan.name} fill sizes="52px" style={{objectFit:"cover"}}/></div>
            ...
            {product.stock !== undefined && (
              <div className="mb-3">
                <span style={{fontSize:'0.83rem',fontWeight:600,
                  ...
                  {product.stock > 5 ? 'متوفر في المخزون'
                    : product.stock > 0 ? `آخر ${product.stock} قطع!`
                    : 'نفذ المخزون'}
                </span>
              </div>
            )}
            ...
            <div className="d-flex align-items-center gap-3 mb-3 flex-wrap">
              <div className="d-flex align-items-center" style={{border:'1.5px solid var(--stone)',borderRadius:10,overflow:'hidden'}}>
                ...
                <span style={{padding:'8px 20px',fontWeight:600,fontSize:'1rem',borderLeft:'1px solid var(--stone)',borderRight:'1px solid var(--stone)'}}>{qty}</span>
              ...
              <button onClick={handleWishlist} className="btn d-flex align-items-center justify-content-center"
                style={{borderRadius:10,fontWeight:700,padding:'12px 14px',border:'1.5px solid var(--stone)',color:'var(--warm-gray)'}}>
                <i className="bi bi-heart"/>
              </button>
            </div>
            <Link href="/checkout" className="btn btn-outline-primary w-100 d-flex align-items-center justify-content-center gap-2"
          ...
          {isAuth && (
            <div className="ha-card p-4 mb-4">
              ...
              <form onSubmit={handleReview}>
                ...
                <button type="submit" disabled={reviewLoading} className="btn btn-primary" style={{borderRadius:8,fontWeight:600}}>
              ...
              <div key={r._id} className="ha-card p-4 mb-3">
                <div className="d-flex justify-content-between align-items-start mb-2">
                  <div className="d-flex gap-3 align-items-center">
                    {r.user?.avatar
                      ? <div style={{width:36,height:36,borderRadius:"inherit",overflow:"hidden",position:"relative",flexShrink:0}}><Image src={r.user.avatar} alt="" fill sizes="36px" style={{objectFit:"cover"}}/></div>
                ...
                {r.comment && <p style={{margin:0,fontSize:'0.88rem',color:'var(--warm-gray)',lineHeight:1.7}}>{r.comment}</p>}
              </div>
            ))
          )}
        </div>
