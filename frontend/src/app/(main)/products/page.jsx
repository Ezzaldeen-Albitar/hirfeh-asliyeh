...
function ProductsContent() {
  ...
  const [filters, setFilters] = useState({
    craftType:   searchParams.get('craftType') || '',
  ...
  const query = {
    page, sort, limit: 12,
    ...(filters.craftType   && { craftType:   filters.craftType }),
      ...
      <div style={{background:'var(--parchment)',borderBottom:'1px solid var(--gold-pale)',padding:'32px 0'}}>
        <div className="container">
          ...
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
              <h1 style={{fontFamily:'Amiri,serif',fontSize:'2rem',color:'var(--charcoal)',marginBottom:0}}>استكشف الحرف الأردنية</h1>
              {!isLoading && (
                <small style={{color:'var(--warm-gray)'}}>
                  {total > 0 ? `${total.toLocaleString('ar-EG')} منتج متاح` : 'لا توجد منتجات بهذه المعايير'}
                </small>
              )}
            </div>
            ...
            {isLoading || isFetching ? (
              ...
              <div className="text-center py-5" style={{color:'var(--warm-gray)'}}>
                ...
                <button className="btn btn-outline-primary mt-2" style={{borderRadius:8}}
                  onClick={()=>{ setFilters({craftType:'',governorate:'',minPrice:'',maxPrice:'',search:'',artisan:''}); setPage(1); }}>
                  مسح الفلاتر
                </button>
              </div>
            ) : (
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="text-center py-5"><span className="spinner-border" style={{color:'var(--burgundy)'}}/></div>}>
      <ProductsContent/>
    </Suspense>
  );
}
