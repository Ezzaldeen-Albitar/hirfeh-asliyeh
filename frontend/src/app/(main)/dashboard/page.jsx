...
const QUICK_LINKS = [
  ...
  { href:'/customizations',    icon:'bi-tools',      label:'طلبات التخصيص', color:'#8B5CF6'        },
];

export default function CustomerDashboard() {
  const { user } = useAuth();
  const { data, isLoading } = useGetOrdersQuery({ limit:10 });
    ...
    <div className="bg-cream" style={{minHeight:'80vh'}}>
      <div style={{background:'var(--parchment)',borderBottom:'1px solid var(--gold-pale)',padding:'36px 0'}}>
        <div className="container">
          ...
          <p style={{color:'var(--warm-gray)',margin:0,fontSize:'0.9rem'}}>هنا يمكنك متابعة طلباتك ومفضلتك</p>
        </div>
      </div>
          ...
          {QUICK_LINKS.map(a => (
            <div key={a.href} className="col-6 col-md-3">
              <Link href={a.href} className="ha-card p-4 text-center text-decoration-none d-block">
                <div style={{width:52,height:52,borderRadius:14,background:a.color+'18',margin:'0 auto 12px',
                  display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1.5rem',color:a.color}}>
                  ...
                  {orders.map(o => {
                    ...
                    return (
                      <tr key={o._id} style={{borderBottom:'1px solid var(--gold-pale)'}}>
                        ...
                        <td className="px-3">{o.items?.length || 1} منتج</td>
                        <td className="px-3">
