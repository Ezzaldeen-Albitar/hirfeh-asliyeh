...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await login({ phone, password: pass, role }).unwrap();
    ...
    <div className="min-vh-100 d-flex" style={{background:'var(--cream)'}}>
      ...
      <div className="d-none d-lg-block col-lg-6 position-relative overflow-hidden">
        ...
        <div style={{position:'absolute',inset:0,background:'rgba(90,20,34,0.45)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:16}}>
          ...
          <div style={{fontFamily:'Playfair Display,serif',fontSize:'1rem',color:'rgba(255,255,255,.8)',letterSpacing:3,textTransform:'uppercase'}}>
            Hirfeh Asliyeh
          </div>
        </div>
      </div>
      ...
      <div className="col-12 col-lg-6 d-flex align-items-center justify-content-center p-4">
        <div style={{width:'100%',maxWidth:440}}>
            ...
            <div style={{display:'flex',border:'1.5px solid var(--stone)',borderRadius:12,overflow:'hidden'}}>
              {[{k:'customer',l:'مشتري'},{k:'artisan',l:'حرفي'}].map(({k,l}) => (
                <button key={k} type="button"
                  onClick={() => setRole(k)}
                  style={{padding:'8px 28px',border:'none',fontFamily:'Tajawal,sans-serif',fontWeight:600,fontSize:'0.9rem',
                    transition:'all .2s',
          ...
          <form onSubmit={handleSubmit}>
            {/* Phone */}
            <div className="mb-3">
              <label className="form-label fw-500" style={{fontSize:'0.88rem'}}>رقم الهاتف</label>
              <div className="input-group">
                <span className="input-group-text" style={{background:'var(--parchment)',border:'1.5px solid var(--stone)',borderRadius:'8px 0 0 8px',fontSize:'0.85rem'}}>
                  🇯🇴 +962
                </span>
            ...
            <div className="mb-4">
              <div className="d-flex justify-content-between">
                ...
                <Link href="/forgot-password" style={{fontSize:'0.82rem',color:'var(--burgundy)'}}>نسيت كلمة المرور؟</Link>
              </div>
            ...
            <button type="submit" disabled={isLoading}
              ...
              style={{borderRadius:10,fontWeight:700,fontSize:'1rem',letterSpacing:.5}}>
            ...
            <p className="text-center mb-0" style={{fontSize:'0.88rem',color:'var(--warm-gray)'}}>
              ليس لديك حساب؟{' '}
              <Link href="/register" style={{color:'var(--burgundy)',fontWeight:700}}>إنشاء حساب جديد</Link>
            </p>
          </form>
        </div>
      </div>
