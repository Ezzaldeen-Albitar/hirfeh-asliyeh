...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSent(true);
      toast.success('تم إرسال رمز الاسترداد إلى هاتفك');
    } catch { toast.error('تعذر إرسال الرمز، تحقق من رقم الهاتف'); }
  };
        ...
        {sent ? (
          <div>
            <div className="alert" style={{background:'rgba(34,197,94,.1)',border:'1px solid rgba(34,197,94,.3)',borderRadius:10,color:'#16a34a'}}>
              <i className="bi bi-check-circle-fill me-2"/>تم إرسال رمز الاسترداد بنجاح
            </div>
            <Link href="/verify-otp" className="btn btn-primary w-100 py-3" style={{borderRadius:10,fontWeight:700}}>
              إدخال الرمز
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit}>
            <p style={{color:'var(--warm-gray)',fontSize:'0.88rem',marginBottom:24}}>
              أدخل رقم هاتفك وسنرسل لك رمز الاسترداد
            </p>
            ...
            <Link href="/login" style={{fontSize:'0.85rem',color:'var(--warm-gray)'}}>
              <i className="bi bi-arrow-right me-1"/>العودة لتسجيل الدخول
            </Link>
          </form>
        )}
      </div>
